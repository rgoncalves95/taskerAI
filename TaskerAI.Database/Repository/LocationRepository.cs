namespace TaskerAI.Database.Repository
{
    ...
    internal class LocationRepository : ILocationRepository
    {
        ...
        public LocationRepository(IDbConnection db) => this.db = db;

        public Domain.Entities.Location Create(Domain.Entities.Location domainEntity)
        {

            Entities.Location newLocation = Mapper.map(domainEntity);

            long id = this.db.Insert(newLocation);
        ...
        {
            string sql = "SELECT * FROM Locations WHERE alias = @Alias or tags = @Tags";
        ...
        public Domain.Entities.Location Get(string latitude, string longitude, string door, string floor)
        {
            ...
            string sql = "SELECT * FROM Locations WHERE latitude = @Latitude and longitude = @Longitude and door = @Door and floor = @Floor ";
            Entities.Location result = this.db.Query<Entities.Location>(sql, new Entities.Location { Latitude = latitude, Longitude = longitude, Door = door, Floor = floor }).FirstOrDefault();
        ...
        {
            Entities.Location location = this.db.Get<TaskerAI.Database.Entities.Location>(id);
